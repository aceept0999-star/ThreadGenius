...
with tab1:
    ...
    if news_source_type == "RSSフィードから自動取得":
        ...
        # ✅ 置換②：テンプレ選択（完成版6種 + 1テーマ5役割テンプレ）＋起業家/店舗ペルソナ自動連動
...
【目的】返信（会話）を増やし、コメント欄で状況を聞き出せる投稿にする。""",
            ...
            "🧩1テーマ5役割｜店舗：予約が増えない（被り防止）": """【テーマ】投稿はしてるのに予約が増えない（原因は“回数”より“入口〜予約までの道”）
【前提】同じテーマで“1日5投稿”作るが、文章の被りは禁止。5本は必ず別の型で。

【あなたへの指示】以下の5役割で、Threads投稿を5本作成すること（各500字以内）。
①共感（あるある）：現場の忙しさ/あるある→「当てはまる？Yes/No」で終える
②診断（番号回答）：予約が止まるポイント5択→「1〜5どれ？」で終える
③今日の1手（超具体）：今日10分でできる改善を1つ→「どれからやる？」で終える
④事例/たとえ：改善前→改善後が想像できる話→「あなたはどのパターン？」で終える
⑤誤解を壊す：よくある勘違いを否定→「賛成/反対？」で終える

【診断軸（5択で使用）】
1 入口（Googleマップ/MEO/検索/SNSのどこで見つかる？）
2 写真（雰囲気・メニュー・実績が伝わる？）
3 比較不安（料金/流れ/所要時間/初回の不安が消える？）
4 予約導線（予約ボタン・リンク・電話が迷子になってない？）
5 口コミ/信頼（声・評価・返信で安心材料が積めてる？）
